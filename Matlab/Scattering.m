...
Err_p=abs(tand(theta4(1:Nmax))-((sind(thetacm_p))./(gcm*(-cosd(thetacm_p)+(vcm*E4cm/P4cm)))));
Err_m=abs(tand(theta4(1:Nmax))-((sind(thetacm_m))./(gcm*(-cosd(thetacm_m)+(vcm*E4cm/P4cm)))));
flag=0;
for(i=1:Nmax)
...
subplot(2,2,4)
plot(theta4(1:Nmax),Q_p,'g',theta4(1:Nmax),Q_m,'b')
xlabel('\theta3')
ylabel('Q_value')
end
if(BOTH==2 || BOTH==3)
